...
| `models-metadata.conf` | Display names and context limits for OpenCode (auto-generated) |
...
> **Note**: `models-metadata.conf` is auto-generated from `models-metadata.conf.example` on first run. If the example file is updated, setup scripts will offer to overwrite your local copy (with backup) or show a diff.
