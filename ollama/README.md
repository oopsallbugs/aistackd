...
> **Note**: For AI coding assistants like OpenCode that require tool calling, consider using [llama.cpp](../README.md) instead. Ollama's tool calling support is limited or broken in most cases.
...
```bash
./sync-opencode-config.sh              # Replace config
./sync-opencode-config.sh --merge      # Add new models only
./sync-opencode-config.sh --dry-run    # Preview changes
```
